...
If you stick around here, I've shown some of the notebook's results below.
...
These charts account for spreads, and in fact I've been particularly conservative in my calculations, using the worst spread I could find for a given moneyline.
...
- The "I Will Die If I Lose Money" charts on the right have smaller, but still _highly real_ blue zones. It will be harder to find a game there, and the payoff will be _slightly_ worse, but you _cannot lose money_.
- The second row is _much_ bluer than the first. Promotions (most famously Barstool and Fanduel) where a winning Bet Token also returns its stake are _roughly twice as valuable_.
- If you look at the same point across different charts, the importance of risk-adjustment becomes particularly obvious. The optimal odds from the top-left chart (260, 370) give a 34.1% risk-unadjusted profit, but one chart over, the very same point has a risk-adjusted profit of _1.5%_ - almost pointless. (Two charts over, the risk-adjusted profit is negative one-bajillion percent: there is a non-zero chance of losing some money, and so the risk-adjustment drops you to oblivion).

Choose whichever chart best describes your situation, and set sail for your Island of Blue Money!
...
The astute reader will note that the optimal odds in this scenario would have been (+370, 100), the payoff where we landed (+270, -110) was only ~2% smaller, and the accounting is slightly simpler.
