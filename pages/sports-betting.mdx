...
<div style={{ width: "30px", height: "2px", backgroundColor: "darkgray" }} />
...
To recap: there are three paths we could take through the promotion: _Win_, _Loss -> Win_, _Loss -> Loss_.
...
Maybe the most intuitive way to value this puddle of probabilities is to average the payouts at the end of each path, weighting by how likely we are to follow them.
There's a 50% chance of walking down the _Win_ path, where $2000 is waiting for us. There's a 25% chance each of going down _Loss -> Win_ and _Loss -> Loss_, which offer $1000 and $0 respectively.

Combining this together, we have: (50% _ 2000) + (25% _ 1000) + (25% \* 0) = $1250
...
Keep track of the plethora of paths, payoffs and probabilities is potentially problematic: it can be helpful to draw things out.

![](/images/diagram1.png)

From a certain perspective, the MGM promo is a way of turning $1000 real dollars into $1250 expected dollars, i.e. it has an expected profit of only **$250**.
...
It's in situations which _do_ involve potential loss, like the MGM promo, that the team-up strategy truly shines.
...
positive ERDs. I've also switched from representing absolute profit (_"You will make
250 ERDs"_) to percentage profit, because different promotions have different limits,
you might be nervous to max out a promotion, etc.

If the giant red-blue squares look
imposing, fear not: we'll walk through an example.
...
If we max out the MGM promo, that's 193 ERD. Not bad, but if we nudge over to (+140, +200) we can reach the heady heights of 263 ERD.
